...
EdgeEvent.prototype.handleEvent = function handleEvent (ev) {
  ...
  if (start.x < this.value.gridPadding) {
    this.dragEdge("down", start);

  } else if (start.x > (ev.target.clientWidth - (2 * this.value.gridPadding))) {
    this.dragEdge("right", start);

  } else if (start.y < this.value.gridPadding) {
    this.dragEdge("top", start);

  } else if (start.y > (ev.target.clientHeight - (2 * this.value.gridPadding))) {
    ...
    switch (edgeDir) {
      case "right":
        this.fn({
          dim: 0,
          shape: start.shape[0]
            + Math.floor(delta.x / this.value.itemWidth),
        });
        break;
      case "bottom":
        this.fn({
          dim: 1,
          shape: start.shape[1]
            + Math.floor(delta.y / this.value.itemHeight),
        });
        break;
    }
  }.bind(this);
